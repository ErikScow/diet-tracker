...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    updateApiLoginError: (state, action) => {
      state.apiLoginError = action.payload;
    },
    updateApiRegisterError: (state, action) => {
      state.apiRegisterError = action.payload;
    },
    updateUserUpdateError: (state, action) => {
      state.apiUserUpdateError = action.payload;
    },
    toggleUnderRecommended: (state, action) => {
      state.underRecommended = !state.underRecommended;
    },
  },
});
...
export const loginCall = (loginCredentials, onSuccess) => (dispatch) => {
  ...
  apiCalls
    ...
    .then((res) => {
      ...
      res.data.userInfo.age =
        parseInt(date.slice(0, 4)) -
        parseInt(res.data.userInfo.birth_date.slice(0, 4));
...
export const registerCall =
  (registerInfo, loginCredentials, onLoginSuccess) => (dispatch) => {
